...
async def _require_private_for_registration(message):
    """Registration is allowed only in the bot's private chat."""
    chat = message.chat
    if chat.type != "private":
        await message.answer(
            "Привет👋\n\n"
            "Зарегистрируйся в личных сообщениях бота.\n"
            "Открой личку с ботом и отправь /start"
        )
        return False
    return True
...
async def cmd_start(message: types.Message, state: FSMContext):
    if not await _require_private_for_registration(message):
        return
    user = db.get_user(message.from_user.id)
    if user:
...
async def cb_list_orgs(callback: types.CallbackQuery):
    ...
    for po in player_orgs:
        parts.append(_render_player_org_card(po["id"]))
...
def _render_player_org_card(org_id: int) -> str:
    ...
    non_owners = [m for m in members if m[0] != owner_uid]
    if non_owners:
        lines = []
        for m in non_owners:
            u = db.get_user(m[0])
            uname = f"@{u[1]}" if u and u[1] else str(m[0])
            game = f" — {u[3]}" if u and u[3] else ""
...
async def player_invite_to_org(message: types.Message):
    ...
    if org["owner_uid"] != message.from_user.id:
        ...
        return
    if db.is_player_org_member(org["id"], target[0]):
        await message.answer(f"❌ Игрок уже состоит в «{org['name']}».")
        return
    db.add_player_org_member(org["id"], target[0])
...
async def player_org_info(message: types.Message):
    ...
    if not org:
        await message.answer(f"❌ Организация «{org_name_raw}» не найдена.")
        return
    await message.answer(_render_player_org_card(org["id"]), parse_mode="HTML")
...
async def cb_credit_pay_all(callback: types.CallbackQuery):
    ...
    credits = db.get_credits(uid, active_only=False)
    total_paid = 0
...
REGISTRATION_COMMANDS = {"/register", "/регистрация", "/рег", "/начать", "/создать"}

@dp.message(F.chat.type.in_({"group", "supergroup"}))
async def group_registration_guard(message):
    text = (message.text or "").strip().lower().split()[0] if (message.text or "").strip() else ""
    if text in REGISTRATION_COMMANDS:
        await message.answer(
            "Привет👋\n\n"
            "Зарегистрируйся в личных сообщениях бота.\n"
            "Открой личку с ботом и отправь /start"
